...
class AuthService extends BaseApiService {
    async login(credentials) {
        try {
            const response = await this.post('/auth/login', credentials); // Fixed: pass credentials and use 'this' instead of 'apiService'
...
class ApiServiceManager {
    constructor() {
        ...
        this.bloodline = new BloodlineService();
            ...
            async (error) => {
                ...
                if (error.response && error.response.status === 401 && !originalRequest._retry) {
                    ...
                    try {
                        ...
                        // Update token in service instances
                        this.auth.setToken(newToken);
                        this.user.setToken(newToken);
                        this.bloodline.setToken(newToken);
                        ...
                        // Clear tokens from service instances
                        this.auth.clearToken();
                        this.user.clearToken();
                        this.bloodline.clearToken();
